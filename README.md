# Adverse Event Reporting SOP  
*A complete technical writing project demonstrating pharmacovigilance and regulatory documentation skills.*

---

## Overview  
This project is a fully developed **Standard Operating Procedure (SOP)** that outlines the end-to-end workflow for **Adverse Event (AE) and Adverse Drug Reaction (ADR) reporting** in compliance with international pharmacovigilance regulations (Health Canada, FDA, EMA, MHRA, CDSCO, WHO).

The document demonstrates expertise in:
- Technical writing  
- Pharmacovigilance processes  
- SOP structure and formatting  
- Regulatory terminology  
- MedDRA coding principles  
- Case processing workflows  


## Skills Demonstrated
- SOP writing & documentation  
- Healthcare/Pharma technical writing  
- Process writing & workflow mapping  
- Medical terminology  
- Regulatory compliance (ICH E2A/E2B)  
- Pharmacovigilance fundamentals  
- Case processing & safety reporting  


## Project Files
- **Adverse-Event-Reporting-SOP.pdf** — Full SOP (Purpose, Scope, Definitions, Templates, QC, Example Case)
- **README.md** — Portfolio summary  

## SOP Table of Contents  
This SOP includes 17 professionally structured sections:

- Purpose  
- Scope  
- Definitions & Abbreviations  
- Responsibilities  
- Minimum Criteria for a Valid Case  
- Sources of Adverse Events  
- Case Intake Process  
- Serious Case Escalation  
- Safety Database Entry  
- Causality Assessment  
- Quality Control (QC)  
- Reporting Requirements  
- Record Management  
- Templates (Narrative, Follow-up, Escalation Email)  
- Example Case  
- Compliance Notes  
- Revision History  


## Relevant For Roles
This project is ideal for showcasing portfolio skills for:

- **Technical Writer**  
- **Pharmacovigilance Associate**  
- **Regulatory Writer**  
- **Medical Writer (Entry Level)**  
- **Documentation Specialist**  
- **Healthcare Tech Writer**  

## Preview  
![IMG_2018](https://github.com/user-attachments/assets/7b93bb1f-2a47-4852-b80a-88f2c9d2f8c7)


## Author  
**Harmeet Kaur**  
Technical Writer | Pharmacovigilance | Regulatory Documentation | Healthcare Tech  
Bilingual: English & French  


## Portfolio Note  
This project demonstrates the ability to create **industry-standard SOPs**, showing both **technical writing skill** and **pharmacovigilance domain knowledge**, making it a strong portfolio sample for technical writing and PV roles.
